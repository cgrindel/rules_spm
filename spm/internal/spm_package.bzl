...
def _declare_clang_target_files(ctx, target, build_config_dirname, modulemap_dir_path):
    ...
    # public_hdrs = []
    # for src in ctx.files.srcs:
    # public_hdrs = [f for f in ctx.files.srcs if f.path.endswith()]
    ...
    # # The modulemap files that are generated by SPM will have paths with
    # # absolute paths that are no longer valid (i.e. generated inside of the
    # # Bazel sandbox). Hence, we need to generate new ones with relative paths.
    # if src_modulemap:
